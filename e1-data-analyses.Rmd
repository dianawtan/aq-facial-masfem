...
## female faces
femorg <- select(dataForg, starts_with("Rater"), one_of("aveRating"))  # select rater scores and average ratings
ICC(femorg)  # reported ICC2k average random raters

alpha1 <- alpha(femorg) #%>%  # calculate Cronbach's alpha  >> ************* IS THIS USED??
rownames(alpha1[["item.stats"]])
filter(alpha1[["item.stats"]], "r.col"  > .20)


## male faces
masorg <- select(dataMorg, starts_with("Rater"), one_of("aveRating"))  # select rater scores and average ratings
ICC(masorg)  # reported ICC2k average random raters

alpha(masorg) %>%  # calculate Cronbach's alpha  >> ************* IS THIS USED??
  filter(., "r.col" < .20) %>% 
  glimpse
